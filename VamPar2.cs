...
internal static class Par2Writer
{
    ...
    private static byte[] LE64(long  v) => BitConverter.GetBytes(v);
    private static byte[] LE32(uint  v) => BitConverter.GetBytes(v);
    private static byte[] Pad4(byte[] b)
    {
        int pad = (4 - b.Length % 4) % 4;
        if (pad == 0) return b;
        byte[] out_ = new byte[b.Length + pad];
        Buffer.BlockCopy(b, 0, out_, 0, b.Length);
        return out_;
    }
}
...
internal static class GF16
{
    ...
    public static ushort Pow(int e) => Exp[(e % 65535 + 65535) % 65535];
